...
%% Using Timoshenko theory to calculate FRFs of uniform cross section Solid Cylindrical Beam in free-free condition (Numerical method)
% timo_free_free.m
% T. Schmitz (8/20/04)
% This program uses n Timoshenko beam elements to determine end receptances for free-free beam.
% Input variables are: f, frequency, Hz; EI, structural rigidity including hysteretic damping, N-m^2;
% L, beam length, m; density, kg/m^3; A, cross sectional area, m^2; kp, shear factor; G, shear modulus, N/m^2;
% rg, radius of gyration; n, number of elements

l = L/n;                            % length of each finite element, l
phi = 12 * EI / (kp * AG * l^2);    % shear deformation parameter

%% Single element matrices for Timoshenko beam

% Refer to: Schmitz, T. L., & Duncan, G. S. (2005). 
...
% Calculate required direct and cross receptances for ends of beam
t1 = length(f);
R11 = zeros(2,2,t1);
R21 = zeros(2,2,t1);
R12 = zeros(2,2,t1);
