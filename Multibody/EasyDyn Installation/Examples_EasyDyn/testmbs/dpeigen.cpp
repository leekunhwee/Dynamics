...

{
  ...
  nbrdof=2;
  nbrbody=2;
  application=new char[20]; strcpy(application, "dpeigen");
  InitEasyDynmbs();
  ...
  thescene.CreateVolFile("dpeigen.vol");

  // Let's calculate the poles
  cout << "Eigen analysis of the double pendulum" << endl;
  ComputePoles();
  cout << "Eigen values computed !" << endl;
  // Let's create the images of the modes for futher animation with EasyAnim
  CreateVmoFile(thescene);
  cout << "Animation files created !" << endl;
