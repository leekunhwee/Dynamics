...
// The purpose of this example is to test the normal contact force
...
body[0].T0G=Tdisp(0,0,q[0])*Trotx(0.9);
body[0].omega.put(0,0,0);
body[0].omegad.put(0,0,0);
body[0].vG.put(0,0,qd[0]);
body[0].aG.put(0,0,qdd[0]);
}
...

{
  ...
  nbrdof=1;
  nbrbody=1;
  application=new char[20]; strcpy(application, "tyre1");
  InitEasyDynmbs();
  ...
  thescene.CreateVolFile("tyre1.vol");

  // Let's open the animation file
  VanFile.open("tyre1.van");
