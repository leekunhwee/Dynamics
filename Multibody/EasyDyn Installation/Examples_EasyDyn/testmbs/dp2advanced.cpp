...
#define EASYDYNMBSADVANCED // to declare the global variables
...
// Kinematics for body 0
body[0].omegapartial[0].put(0,0,1);
vec OG1=body[0].T0G.e;
body[0].vGpartial[0]=(body[0].omegapartial[0]^OG1);
// Kinematics for body 1
body[1].omegapartial[0].put(0,0,1);
body[1].omegapartial[1].put(0,0,1);
vec AG2=body[1].T0G.R*vcoord(0,-0.5*l2,0);
body[1].vGpartial[0]=(body[1].omegapartial[0]^(2*OG1+AG2));
body[1].vGpartial[1]=(body[1].omegapartial[1]^AG2);
}
...

{
  ...
  nbrdof=2;
  nbrbody=2;
  application=new char[20]; strcpy(application, "dp2advanced");
  InitEasyDynmbs();
